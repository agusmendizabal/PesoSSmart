...
export function GoalsSection({ userId, projectedMonthlyFree }: GoalsSectionProps) {
  const { goals, addGoal, addToGoal, deleteGoal, isLoading } = useGoalsStore();
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<GoalFormData>({
    ...
    defaultValues: { title: '', target_amount: '', deadline: '', emoji: '🎯' },
  });
  ...
  return (
    <View style={styles.container}>
      ...
      {goals.length === 0 ? (
        ...
        goals.map((goal) => <GoalCard key={goal.id} goal={goal} projectedMonthlyFree={projectedMonthlyFree} onAddFunds={() => setShowAddFundsModal(goal)} onDelete={() => handleDelete(goal)} />)
      )}
...
function GoalCard({
  ...
  const monthsToGoal =
    projectedMonthlyFree != null && projectedMonthlyFree > 0
  ...
  return (
    <Card style={styles.goalCard}>
      ...
      <View style={styles.goalAmounts}>
        ...
        <Text variant="label" color={isComplete ? colors.neon : colors.text.secondary}>
          {Math.round(pct * 100)}%
        </Text>
        <Text variant="caption" color={colors.text.secondary}>
          {formatCurrency(goal.target_amount)} total
        </Text>
      </View>
      ...
      {!isComplete && (
        <View style={styles.goalFooter}>
          {monthsToGoal != null ? (
            <Text variant="caption" color={colors.text.tertiary}>
              A tu ritmo actual: {monthsToGoal} {monthsToGoal === 1 ? 'mes' : 'meses'} para lograrlo
            </Text>
          ) : (
            <Text variant="caption" color={colors.text.tertiary}>
